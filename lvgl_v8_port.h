 ...
 */
/* Override: use mode 0 to avoid heap_caps_free crash with LCD frame buffers (board may set 1/2/3 via sdkconfig). */
...
/* Allow mode 0 to avoid heap_caps_free crash when LCD frame buffers are used with avoid-tear (use heap buffers instead). */
...
#else
    /* Mode 0: avoid-tear disabled; buffers allocated with heap_caps_malloc in port (no #error). */
    #if LVGL_PORT_AVOID_TEARING_MODE != 0
    #error "Invalid avoid tearing mode, please set macro `LVGL_PORT_AVOID_TEARING_MODE` to 0 or 1, 2, 3"
    #endif
#endif
// Check rotation
